...
If output contains `SKILLS_UPDATE_AVAILABLE`: use AskUserQuestion to ask if they want to update now. If yes, run the git pull command shown in the output, then run `bun "$(git -C ~/.claude/skills/lyra-update rev-parse --show-toplevel)/scripts/build.ts"` to recompile, then re-symlink any new skills into `~/.claude/skills/`.
